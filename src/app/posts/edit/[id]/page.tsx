...
export default function EditPostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to post detail page using slug
      const updatedPost = response?.data || response;
      const slug = updatedPost?.slug || postId;
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-3xl">
      <Card className="bg-white border-gray-200">
        <CardHeader>
          <CardTitle className="text-gray-900">Edit Post</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button type="submit" disabled={loading || uploadingImage} className="bg-gray-900 hover:bg-black text-white">
                {uploadingImage ? 'Uploading image...' : loading ? 'Saving...' : 'Update Post'}
              </Button>
            </div>
          </form>
        </CardContent>
